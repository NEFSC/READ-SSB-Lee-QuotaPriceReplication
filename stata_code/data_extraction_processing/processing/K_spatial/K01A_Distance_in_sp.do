...
There are 17*17*12=3,468 observations. 

You have to expand to quarterly data. Once you do this, you have 
...
keep if dateq>=yq(2011,4) & dateq<=yq(2014,1)
*/
...
keep id1 id2 stock1 dateq land rev catch swt snum N1_land N2_land N1_rev N2_rev N1_catch N2_catch N1_swt N2_swt N1_snum N2_snum N1A_land N2A_land N1A_rev N2A_rev N1A_catch N2A_catch N1A_swt N2A_swt N1A_snum N2A_snum 
...
timer on 2
reshape wide land rev catch swt snum N1_land N2_land N1_rev N2_rev N1_catch N2_catch N1_swt N2_swt N1_snum N2_snum N1A_land N2A_land N1A_rev N2A_rev N1A_catch N2A_catch N1A_swt N2A_swt N1A_snum N2A_snum, i(id1) j(id2)
spset id1


order id1 stock1 dateq _ID land* rev*  catch* swt* snum* N1_land* N2_land* N1_rev* N2_rev* N1_catch* N2_catch* N1_swt* N2_swt* N1_snum* N2_snum* N1A_land* N2A_land* N1A_rev* N2A_rev* N1A_catch* N2A_catch* N1A_swt* N2A_swt* N1A_snum* N2A_snum* 

timer off 2

pause
/* spmatrix normalize can change the normalization and I can move back and forth between spectral and min-max. This means I don't really have to store all of these matrices. 

I can store just "raw" and renormalize on the fly. This might be dangerous though.  */
...
foreach metric in land rev catch swt snum{
	...
	*/
	...
	spmatrix fromdata WiiS_`metric'=`metric'`firstid1'-`metric'`lastid1', idistance normalize(spectral)
	spmatrix note WiiS_`metric' : spectral normalized inverse distance  matrix on `metric'
	spmatrix save WiiS_`metric' using ${data_main}/spatial_weight_matrices/WiiS_`metric'.stswm, replace
	
	/* Distance, spectral normalized */
	spmatrix fromdata W00S_`metric'=`metric'`firstid1'-`metric'`lastid1', normalize(spectral)
	spmatrix note W00S_`metric' : non-normalized distance  matrix on `metric'
	spmatrix save W00S_`metric' using ${data_main}/spatial_weight_matrices/W00S_`metric'.stswm, replace
	...
	spmatrix fromdata W000_`metric'=`metric'`firstid1'-`metric'`lastid1', normalize(none)
	spmatrix note W000_`metric' : non-normalized distance  matrix on `metric'
	spmatrix save W000_`metric' using ${data_main}/spatial_weight_matrices/W000_`metric'.stswm, replace
	
	/* put this D matrix into mata */
	spmatrix matafromsp D id =W000_`metric'
	
	/* Inverse Distance, also not normalized
	Inverse distance is smart enough to set the elements=0 if the underlying value =0*/
	spmatrix fromdata Wii0_`metric'=`metric'`firstid1'-`metric'`lastid1', idistance normalize(none)
	spmatrix note Wii0_`metric' : non-normalized inverse distance  matrix on `metric'
	spmatrix save Wii0_`metric' using ${data_main}/spatial_weight_matrices/Wii0_`metric'.stswm, replace

	
	
	
	/* put this ID matrix into mata */
	spmatrix matafromsp W id = Wii0_`metric'
    mata: W = W:^2
	/* construct the ID^2, spectral normalized */
	spmatrix spfrommata Wi2S_`metric'=W id, normalize(spectral)
	spmatrix note Wi2S_`metric' : spectral normalized inverse distance  matrix on `metric'
	spmatrix save Wi2S_`metric' using ${data_main}/spatial_weight_matrices/Wi2S_`metric'.stswm, replace
	...
	spmatrix spfrommata WiiJ_`metric'=W2 id, normalize(none)
	spmatrix note WiiJ_`metric' : Joint spectral normalized inverse distance  matrix on `metric'
	spmatrix save WiiJ_`metric' using ${data_main}/spatial_weight_matrices/WiiJ_`metric'.stswm, replace

	
	spmatrix spfrommata W00J_`metric'=D2 id, normalize(none)
	spmatrix note W00J_`metric' : Joint spectral normalized distance  matrix on `metric'
	spmatrix save W00J_`metric' using ${data_main}/spatial_weight_matrices/W00J_`metric'.stswm, replace
	...
	spmatrix fromdata WiiR_`metric'=`metric'`firstid1'-`metric'`lastid1', idistance normalize(row)
	spmatrix note WiiR_`metric' : row normalized inverse distance  matrix on `metric'
	spmatrix save WiiR_`metric' using ${data_main}/spatial_weight_matrices/WiiR_`metric'.stswm, replace
}
...
foreach metric in N1_land N1_rev N1_catch N1_swt N1_snum N2_land N2_rev N2_catch N2_swt N2_snum{
	spmatrix fromdata WN10_`metric'=`metric'`firstid1'-`metric'`lastid1', normalize(none)
	spmatrix note WN10_`metric' : Nearest Neighbor matrix on `metric'
	spmatrix save WN10_`metric' using ${data_main}/spatial_weight_matrices/W00_`metric'.stswm, replace
	
	spmatrix fromdata WN1S_`metric'=`metric'`firstid1'-`metric'`lastid1', normalize(spectral)
	spmatrix note WN1S_`metric' : spectral normalized Nearest Neighbor matrix on `metric'
	spmatrix save WN1S_`metric' using ${data_main}/spatial_weight_matrices/W0S_`metric'.stswm, replace
}
...
foreach metric in N1A_land N1A_rev N1A_catch N1A_swt N1A_snum N2A_land N2A_rev N2A_catch N2A_swt N2A_snum{
	spmatrix fromdata WN1A0_`metric'=`metric'`firstid1'-`metric'`lastid1', normalize(none)
	spmatrix note WN1A0_`metric' : Adjusted Nearest Neighbor matrix on `metric'
	spmatrix save WN1A0_`metric' using ${data_main}/spatial_weight_matrices/WA0_`metric'.stswm, replace
	
	spmatrix fromdata WN1AS_`metric'=`metric'`firstid1'-`metric'`lastid1', normalize(spectral)
	spmatrix note WN1AS_`metric' : spectral normalized Nearest Neighbor matrix on `metric'
	spmatrix save WN1AS_`metric' using ${data_main}/spatial_weight_matrices/WAS_`metric'.stswm, replace
}




/*For the NN matrices, I'm not sure If I need to normalize them. I believe not.*/
/* need to think about what to do for SNEMA winter flounder in the first two years, when there is no allocation */
