...
local logfile "first_stage_exploratory_POP_${vintage_string}.smcl"
...
local graph_opts ttitle("Quarter") xlabel(200(4)240, angle(45))  tlabel(, format(%tqCCYY)) ttitle("")  byopts(note(""))


xtline frac , ytitle("Fraction sold by Permit Banks") `tlines' `graph_opts'
graph export "${my_images}/descriptive/fraction_of_pounds_by_permit_bank_POP.png", as(png) replace width(2000)
...
graph export "${my_images}/descriptive/pounds_soldby_permit_bank_POP.png", as(png) replace width(2000)
...
tempfile pounds_by_stock
save `pounds_by_stock', replace

xtset stockcode quarterly
xtline pounds, ytitle("Millions of Pounds of Quota Sold") `tlines' `graph_opts'
graph export "${my_images}/descriptive/quota_pounds_transacted_POP.png", as(png) replace width(2000)
...
local tlines tline(200(4)240, lpattern(dash) lcolor(gs12) lwidth(vthin))
local graph_opts ttitle("Quarter") xlabel(200(4)240, angle(45))  tlabel(, format(%tqCCYY)) ttitle("")  byopts(note("")) ylabel(0(10)50)
xtline total_trades, `tlines' `graph_opts' ytitle("Total Trades")
graph export "${my_images}/descriptive/trades_by_quarter_POP.png", replace as(png)

merge 1:1 quarterly stockcode fy using `pounds_by_stock'



label var pounds "Pounds Transferred"
label var total_trades "Number of Trades"
local graph_opts ttitle("Quarter") xtitle("Quarter") xlabel(200(4)240, angle(45))  tlabel(, format(%tqCCYY)) ttitle("")  byopts(note("")) 
local label_opt1 ytitle("Millions of Pounds Transferred", axis(1))
xtline pounds, addplot(line total_trades quarterly, yaxis(2) xtitle("")) `tlines' `graph_opts' `label_opt1'  
...
graph export "${my_images}/descriptive/`myl'_box_singleQ_POP.png", replace as(png)
...
esttab ., `esttab_opts'
esttab . using ${my_tables}/first_stage_tabulate_POP.tex, `esttab_opts'
...
estpost tabulate fy q_fy, nototal
esttab ., `esttab_opts'
esttab . using ${my_tables}/first_stage_tabulate_swaps_POP.tex, `esttab_opts'
...
estpost tabulate fy q_fy, nototal
esttab ., `esttab_opts'
esttab . using ${my_tables}/first_stage_tabulate_package_swaps_POP.tex, `esttab_opts'
...
/* total volume in packages vs swaps */

gen count_trades=1
collapse (sum) total_lbs count_trades, by(fyq basket fy q_fy)
tsset fyq basket

tsfill, full
replace total_lbs=0 if total_lbs==.
replace count_trades=0 if count_trades==.

bysort fyq: replace fy=fy[_n-1] if fy==.
bysort fyq: replace q_fy=q_fy[_n-1] if q_fy==.
mdesc
assert "`r(miss_var)'"==""

bysort fyq: egen tcount=total(count_trades)

bysort fyq: egen tt=total(total_lbs)
gen fraction_volume_basket=total_lbs/tt
gen fraction_trades_basket=count_trades/tcount

keep if basket==1
tsset fyq


local tlines tline(200(4)240, lpattern(dash) lcolor(gs12) lwidth(vvthin))
local graph_opts xlabel(200(4)240, angle(45))  xmtick(##4) tlabel(, format(%tqCCYY)) ttitle("")  
local label_opts legend(order(1 "Quota Pounds" 2 "Transactions")) ytitle("Fraction of Market Activity") 
tsline fraction_volume_basket fraction_trades_basket , `tlines' `graph_opts' `label_opts'
graph export "${my_images}/descriptive/basket_transaction_fractions_POP.png", replace as(png)


graph box fraction_volume_basket fraction_trades_basket if fyq>=tq(2010q3), over(q_fy) ytitle("Fraction of Market Activity") legend(order (1 "Quota Pounds" 2 "Transactions")) nooutside note("") b1title("Quarter")
graph export "${my_images}/descriptive/byQ_basket_transaction_fractions_POP.png", replace as(png)
