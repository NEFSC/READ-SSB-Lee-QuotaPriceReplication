...
local mylogfile "${my_results}/N02_churdle_margins_preferred.smcl" 
...
margins, dydx(*) predict(psel) asobserved
margins, eyex(quota_remaining_BOQ ) predict(psel) asobserved
/**************************************/
...
/* the marginal effects on the Truncated mean*/
margins, dydx(*) predict(ytrun)
...
/**************************************/
/*Elasticities*/
/* the ystar elasticity. This doesn't mean anything.*/ 
margins, eyex(quota_remaining_BOQ live_priceGDP stock_QR_Index) predict(ycen) asobserved

/* the truncated LHS -- E[y|x, y>=0] */ 
margins, eyex(quota_remaining_BOQ live_priceGDP stock_QR_Index) predict(ytrun) asobserved
/**************************************/
/**************************************/
...
/**************************************/
/* PARTICIPATION Equation*/
/* the marginal effects of a 1 unit changes in log of the dep vars.  A little weird.  
and elasticities on the probability of being positive.*/ 
margins, dydx(*) predict(psel) asobserved
margins, eydx(ln_quota_remaining_BOQ ) predict(psel) asobserved
/**************************************/
...
/* the marginal effects on the Truncated mean
	Not quite right, because this is the effect of a small change in the */
margins, dydx(*) predict(ytrun)

/* the ystar elasticity. This doesn't mean anything.*/ 
margins, eydx(ln_quota_remaining_BOQ ln_live_priceGDP ln_stock_QR_Index) predict(ycen)

/* the truncated LHS -- E[y|x, y>=0] */ 
margins, eydx(ln_quota_remaining_BOQ ln_live_priceGDP ln_stock_QR_Index) predict(ytrun)
...
and elasticities on the probability of being positive.*/ 
margins, dydx(*) predict(psel) asobserved
margins, eydx(* ) predict(psel) asobserved
/**************************************/
...
est table exp_14 exp_15 exp_16, star(0.10 .05 .01 )
est table exp_spatial_8 exp_spatial_12 , star(0.10 .05 .01 )
 ...
 /*
 nehurdle badj_GDP ln_live_priceGDP c.ln_quota_remaining_BOQ  ln_fraction_remaining_BOQ ln_stock_QR_Index  _Iq*, trunc  select(_Iq* ln_fraction_remaining_BOQ c.ln_quota_remaining_BOQ )  vce(robust)
